(function(){
 ...
 function show(n){current=n;steps.forEach(s=>{s.classList.toggle('active',+s.dataset.step===n);if(+s.dataset.step===n){s.classList.remove('step-enter');void s.offsetWidth;s.classList.add('step-enter')}});if(bar)bar.style.width=(n/4*100)+'%';if(label)label.textContent='STEP 0'+n;window.scrollTo({top:0,behavior:'smooth'});}
 ...
 form.addEventListener('submit',async e=>{e.preventDefault();if(!validStep(4))return;const consent=document.getElementById('consent');if(consent&&!consent.checked){consent.reportValidity();return}const submitBtn=form.querySelector('.submit-btn');if(submitBtn){submitBtn.disabled=true;submitBtn.dataset.original=submitBtn.innerHTML;submitBtn.innerHTML='Submitting <span>…</span>';}const r=await fetch('/apply',{method:'POST',body:new FormData(form)});const data=await r.json().catch(()=>({ok:false,message:'Submission failed. Please check your connection and try again.'}));if(!r.ok||!data.ok){if(submitBtn){submitBtn.disabled=false;submitBtn.innerHTML=submitBtn.dataset.original||'Submit Application <span>↗</span>';}alert(data.message||'Submission failed. Please try again.');return}form.style.display='none';success.classList.add('show')});
